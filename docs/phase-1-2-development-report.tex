...
\section{Conclusion}
Through Phase 3 the project has solved the integration problems that normally disappear behind a simulator: version-specific reverse engineering, precise structured observation, rule-aware invalid-action masking, safe actuation of a real desktop application, typed reward and lifecycle semantics, portable trajectory logging, and baseline evaluation validated against the real client. The resulting portfolio narrative is therefore \textbf{reverse engineering $\rightarrow$ structured state estimation $\rightarrow$ rules/action masking $\rightarrow$ safe control $\rightarrow$ Environment v1 $\rightarrow$ learned strategy}. With Environment v1 frozen and live validation recorded, Phase 4 can concentrate on learning experiments without reopening the frozen foundations.
\end{document}
